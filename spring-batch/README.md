...
* Job: 처음부터 끝까지 실행할 수 있는 완전한 일괄 처리 프로세스
  ...
  * 1개 이상의 step 구성
* Step: 일괄처리 작업 내의 작업 단위
  * Chunk-Oriented Step: 데이터를 청크로 처리
  * Tasklet Step  단일 작업을 실행
* Reader: 파일 또는 데이터베이스와 같은 원본에서 데이터를 검색
  * ItemReader : JdbcCursorItemReader, FlatFileItemReader
* Processor: 입력 데이터를 변환하거나 처리
  * ItemProcessor : ItemProcessorAdapter, CompositeItemProcessor
* Writer: 처리된 데이터를 파일 또는 데이터베이스와 같은 대상에 저장
  * ItemWriter: MapJobRepository, JobRepositoryFactoryBean
*   Job Repository:

    배치 작업에 대한 메타데이터를 저장(작업 및 단계 실행 데이터 저장)

    작업을 다시 실행 할 수 있음
* JobExplorer : 이는 기본적으로 작업 인스턴스, 실행 및 관련 단계를 검사할 수 있는 쿼리 API.&#x20;
* JobOperator : 작업 시작, 중지 및 쿼리와 같은 작업을 허용하는 더 높은 수준의 API.
