...
<figure><img src="../../.gitbook/assets/image (1) (1).png" alt=""><figcaption><p>사용자 수집</p></figcaption></figure>
...
이메일, SMS, PUSH는 많은 사용자에게 메세지를 전달하는 기능으로 여러 유형의 템플릿을 만들어서 유형에 맞는 메세지를 사용자에게 보내면 일관된 메시지를 보낼 수 있습니다. 메세지안에는 이름, 특별한 문구등 동적으로 반영되는 메세지를 생성하기 위해서 `${}[`표현 언어 (Expression Language, EL)]과 같은 것을 만들어 바인딩 합니다.
...
<figure><img src="../../.gitbook/assets/image.png" alt=""><figcaption><p>일반적인 재처리</p></figcaption></figure>

* 서비스 서버 오류: 서비스 서버 오류의 유형은 많이 있지만 일반적으로 서비스 서버에서 오류가 발생하면 즉시 재처리를 하지 않고 배치(or 데몬)을 사용해서 재처리 합니다.
* 서비스 제공자 관련 오류: 서비스 제공자에 위한 오류는 일반적으로 다음과 같은 오류가 발생합니다.
  * 서비스 서버와 제공자 연동 오류: 서비스 서버에서 제공자 서비스로 전달 중 오류가 발생 하면 정책에 따라 즉시 재 처리를 수행합니다. 몰론 재처리 횟수를 지정하여 오류가 지정한 횟수 보다 많으며 해단 건은 재처리를 하지 않고 배치(or 데몬)을 사용해서 재처리 합니다.
  * 제공자 서비스 내부 오류: 제공자 서비스에서 사용자에 발송이 되지 못하는 상황에 서비스 서버로 결과를 주지 못하면 재처리 방법은 최종 사용자가 메세지 수신시 돌아오는 정보를 기준으로 재처리 할 수 있지만 돌아오는 정보가 없다면 재처리 방법은 없습니다. 만약 제공자 서비스가 결과를 서비스 서버로 준다면 정책에 따라 실시간 재처리 하거나 배치(or 데몬)을 사용해서 재처리 합니다.
