...
온도, 습도, 광량, 이산화탄소 농도 등 홤경 데이터, 토양에 대한 습도, 온도,  pH, 전기 전도도, 산소 농도등  데이터 및 장비의 상태 테이터, 제어 테이터를 세분화 하여 메타 데이터로 관리합니다. 예를 들면 딸기 작물에 대해 날짜, 시간,  배지무게(kg), 내부CO2(ppm), 지습(%), 토양PH(ph), (양액)공급량(cc) (양액)배액량(cc), 외부온도(도), 내부온도(도) 등의 테이터를

에 대한 장비와 장비의 상태 및 상태 정보를 메
